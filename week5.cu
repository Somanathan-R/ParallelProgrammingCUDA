...
__global__ void globalMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    int span = (numElements + numThreads - 1) / numThreads; // Calculate the span of a thread's search

    if (threadId < numThreads)
    {
        int start = threadId * span;
        int end = min(start + span, numElements);

        for (int i = start; i < end; ++i)
        {
            atomicAdd(&input[i], 1); // Increment the associated memory input value by 1
...
__global__ void sharedMemorySearch(int *input, int totalFound, int numElements, int numThreads)
{
    ...
    if (threadId < numThreads)
    {
        ...
        {
            atomicAdd(&input[i], 1); // Increment the associated memory input value by 1
            if (input[i] == constant_search_value) // Check if the value matches the constant search value
            {
                atomicAdd(&totalFound, 1); // Increment the total found count
                // Optionally, you can set the value in the input array for the associated index to 1
            }
        }
    }
}
...
{
    int threadId = blockDim.x * blockIdx.x + threadIdx.x;
    int span = (constant_num_elements + constant_num_threads - 1) / constant_num_threads; // Calculate the span of a thread's search

    if (threadId < constant_num_threads)
    {
        int start = threadId * span;
        int end = min(start + span, constant_num_elements);

        for (int i = start; i < end; ++i)
        {
            if (constant_input[i] == constant_search_value) // Check if the value matches the constant search value
            {
                atomicAdd(&totalFound, 1); // Increment the total found count
                // Optionally, you can set the value in the input array for the associated index to 1
            }
...
__host__ void executeKernel(int *d_input, int numElements, int threadsPerBlock, std::string kernelType)
{
    ...
    {
        globalMemorySearch<<<1,threadsPerBlock>>>(d_input, totalFound, numElements, threadsPerBlock);
    ...
    } else if (!strcmp(kernelType.c_str(), "shared"))
    {
        ...
        sharedMemorySearch<<<1,threadsPerBlock, int_array_size>>>(d_input, totalFound, numElements, threadsPerBlock);
    } else {
        registerMemorySearch<<<1,threadsPerBlock>>>(d_input, totalFound, numElements, threadsPerBlock);
    }
